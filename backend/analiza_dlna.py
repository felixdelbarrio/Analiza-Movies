...
Alineado con la filosofía de logging centralizada en `backend/logger.py`:
- _logger.progress(...)            -> señal SIEMPRE visible (heartbeat)
- _logger.info/warning/error(...)  -> logs por nivel (respetan SILENT_MODE salvo error())
- _logger.debug_ctx("DLNA", "...") -> debug contextual (DEBUG_MODE gated)

Cambios clave (streaming):
- report_all.csv se escribe incrementalmente (open_all_csv_writer) => no se acumula all_rows.
- metadata_fix.csv se escribe incrementalmente (open_suggestions_csv_writer) => no se acumula suggestions_rows.
- Solo se mantiene en memoria:
    * filtered_rows (DELETE/MAYBE) para ordenarlo al final (si hay)
    * contadores (decisions_count) y contadores globales (procesados/errores)
- report_filtered.csv:
    * SOLO se genera si hay filas (open_filtered_csv_writer_only_if_rows)
    * En SILENT_MODE, el resumen muestra explícitamente:
      filtered_csv=SKIPPED (0 rows) cuando no hay DELETE/MAYBE.

Métricas OMDb:
- Snapshots/deltas del cliente OMDb para debugging en SILENT+DEBUG.
- Deltas por contenedor (scan/analyze) para rankings de “coste”.
"""
...
def _log_omdb_rankings(deltas_by_group: dict[str, dict[str, int]], *, min_groups: int = 2) -> None:
    ...
    if (line := _fmt(_rank_top_by_total_cost(deltas_by_group, top_n=5))):
        _logger.progress(f"[DLNA][DEBUG] Top containers by TOTAL_COST: {line}")
    if (line := _fmt(_rank_top(deltas_by_group, "http_requests", top_n=5))):
        _logger.progress(f"[DLNA][DEBUG] Top containers by http_requests: {line}")
    if (line := _fmt(_rank_top(deltas_by_group, "rate_limit_sleeps", top_n=5))):
        _logger.progress(f"[DLNA][DEBUG] Top containers by rate_limit_sleeps: {line}")
    if (line := _fmt(_rank_top(deltas_by_group, "http_failures", top_n=5))):
        _logger.progress(f"[DLNA][DEBUG] Top containers by http_failures: {line}")
    if (line := _fmt(_rank_top(deltas_by_group, "rate_limit_hits", top_n=5))):
        _logger.progress(f"[DLNA][DEBUG] Top containers by rate_limit_hits: {line}")
...
def _fetch_xml_root(url: str, timeout_s: float = 5.0) -> ET.Element | None:
    try:
        ...
        _logger.warning(f"[DLNA] No se pudo descargar XML {url}: {exc!r}", always=True)
        return None

    try:
        return ET.fromstring(data)
    except Exception as exc:  # pragma: no cover
        _logger.warning(f"[DLNA] No se pudo parsear XML {url}: {exc!r}", always=True)
        return None
...
def _soap_browse_direct_children(
    ...
    try:
        ...
        _logger.error(f"[DLNA] Error SOAP Browse contra {control_url}: {exc!r}")
        return None

    try:
        envelope = ET.fromstring(raw)
    except Exception as exc:  # pragma: no cover
        _logger.error(f"[DLNA] Respuesta SOAP inválida: {exc!r}")
        return None
    ...
    try:
        ...
        try:
            ...
            _logger.error(f"[DLNA] No se pudo parsear DIDL-Lite: {exc!r}")
            return None
...
def _list_root_containers(device: DLNADevice) -> tuple[list[_DlnaContainer], tuple[str, str] | None]:
    ...
    if endpoints is None:
        _logger.error(f"[DLNA] El dispositivo '{device.friendly_name}' no expone ContentDirectory.")
        return [], None
...
def analyze_dlna_server(device: DLNADevice | None = None) -> None:
    ...
    _logger.debug_ctx("DLNA", f"location={device.location!r}")
    ...
    if base.object_id != chosen_root.object_id:
        _logger.debug_ctx("DLNA", f"auto_descend: {chosen_root.title!r} -> {base.title!r}")
    ...
    if SILENT_MODE:
        _logger.progress(
            f"[DLNA] ThreadPool workers={max_workers} (por contenedor) "
        ...
        _logger.debug_ctx(
            "DLNA",
            f"ThreadPool workers={max_workers} (por contenedor, cap por OMDb limiter) "
            f"OMDB_HTTP_MIN_INTERVAL_SECONDS={OMDB_HTTP_MIN_INTERVAL_SECONDS}",
        )

    # Solo mantenemos en memoria las filas filtradas para ordenarlas al final (si hay).
    filtered_rows: list[dict[str, object]] = []

    # Contadores (sin all_rows)
    ...
    def _analyze_one(
        *,
        ...
        # file “friendly” estable siempre
        if row:
            row["file"] = display_file
            row["file_url"] = file_url  # quedará ignorado por CSV (extrasaction="ignore"), pero útil en memoria
    ...
    with open_all_csv_writer(REPORT_ALL_PATH) as all_writer, open_suggestions_csv_writer(METADATA_FIX_PATH) as sugg_writer:
        # ---------------------------------------------------------------------
        # NO SILENT: necesitamos total real para (i/total) => materializamos items
        # ---------------------------------------------------------------------
        if not SILENT_MODE:
            ...
            for idx, container in enumerate(selected_containers, start=1):
                _logger.progress(f"[DLNA] Escaneando contenedor ({idx}/{total_containers}): {container.title}")

                try:
                    items = _iter_video_items_recursive(device, container.object_id)
                except Exception as exc:  # pragma: no cover
                    scan_errors += 1
                    _logger.error(f"[DLNA] Error escaneando '{container.title}': {exc!r}", always=True)
                    continue

                _logger.debug_ctx("DLNA", f"scan {container.title!r} items={len(items)}")
            ...
            for container_title, items in candidates_by_container.items():
                ...
                with ThreadPoolExecutor(max_workers=min(max_workers, max(1, len(items)))) as executor:
                    for raw_title, resource_url, file_size, library in items:
                        ...
                        results.append(None)
                        future_to_index[fut] = len(results) - 1
                    ...
                    for fut in as_completed(future_to_index):
                        ...
                        try:
                            ...
                            _logger.error(f"[DLNA] Error analizando item (future): {exc!r}", always=True)
                        finally:
        ...
        # ---------------------------------------------------------------------
        # SILENT: streaming por contenedor + métricas por fase
        # ---------------------------------------------------------------------
        else:
            total_containers = len(selected_containers)
            total_candidates = 0
            ...
            for idx, container in enumerate(selected_containers, start=1):
                ...
                _logger.progress(f"[DLNA] Escaneando contenedor ({idx}/{total_containers}): {container_title}")
                ...
                try:
                    ...
                    scan_errors += 1
                    _logger.error(f"[DLNA] Error escaneando '{container_title}': {exc!r}", always=True)
                    continue

                total_candidates += len(items)
                _logger.debug_ctx("DLNA", f"scan {container_title!r} items={len(items)}")
                ...
                _logger.progress(f"[DLNA] ({idx}/{total_containers}) Analizando contenedor: {container_title} (items={len(items)})")
                ...
                with ThreadPoolExecutor(max_workers=min(max_workers, max(1, len(items)))) as executor:
                    ...
                    for it in items:
                        ...
                        if DEBUG_MODE and (analyzed_so_far % _PROGRESS_EVERY_N_ITEMS == 0):
                            _logger.progress(f"[DLNA][DEBUG] Progreso: analizados {analyzed_so_far} items...")
                        ...
                        future_to_item_count[fut] = 1

                    completed = 0
                    for fut in as_completed(future_to_item_count):
                        completed += 1
                        if DEBUG_MODE and (completed % _PROGRESS_EVERY_N_ITEMS == 0):
                            _logger.progress(f"[DLNA][DEBUG] {container_key}: completados {completed}/{len(items)} items...")

                        try:
                            row, meta_sugg, logs = fut.result()
                        except Exception as exc:  # pragma: no cover
                            total_items_errors += 1
                            _logger.error(f"[DLNA] Error analizando item (future): {exc!r}", always=True)
                            total_items_processed += 1
            ...
            _logger.progress(f"[DLNA] Candidatos detectados (streaming): {total_candidates}")
    ...
    _logger.info("[DLNA] Análisis completado.", always=True)