...
def _compute_bayes_score(
    ...
    *,
    ...
    c = float(c_global)
    return (v / denom) * r + (m_eff / denom) * c
...
def compute_scoring(
    ...
    Regla clave:
    - Metacritic NUNCA decide por sí solo; solo refuerza reason.
    """
    try:
        # ------------------------------------------------------------------
        ...
        inputs: ScoringDict = {
            ...
            # score_bayes SIEMPRE presente (consistencia)
            "score_bayes": None,
        }
        ...
        # cap para evitar delete agresivo
        ...
        if bayes_score is not None:
            bayes_score = _clamp_float(float(bayes_score), lo=0.0, hi=10.0)
            inputs["score_bayes"] = bayes_score
        ...
        if (
            ...
            if preliminary_decision == "DELETE":
                ...
                extra = (
                    f"Además RT={rt_score_i}% ≤ RT_DELETE_MAX_SCORE={RT_DELETE_MAX_SCORE}, "
                    "lo que refuerza el borrado."
                )
                return {
                    "decision": "DELETE",
                    "reason": (base + " " + extra).strip(),
            ...
            if preliminary_decision == "MAYBE" and float(bayes_score) <= (
                ...
                return {
                    "decision": "DELETE",
                    "reason": (
                        f"score_bayes={float(bayes_score):.2f} cercano a DELETE={bayes_delete_thr:.2f} "
        ...
        return {
            "decision": "UNKNOWN",
            "reason": "Información parcial no interpretable (por ejemplo, Metacritic sin IMDb/RT, o valores inválidos).",
            "rule": "UNKNOWN_PARTIAL",
            "inputs": inputs,
        }

    except Exception:
        # Fail-safe absoluto: nunca romper pipeline
        return {
            "decision": "UNKNOWN",
            "reason": "compute_scoring: error inesperado (fail-safe).",
            "rule": "FAILSAFE_EXCEPTION",
            "inputs": {
                "imdb_rating": None,
                "imdb_votes": None,
                "rt_score": None,
