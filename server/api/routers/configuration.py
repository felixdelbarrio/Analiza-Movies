...
from server.api.services.analysis_runs import get_run_status, start_profile_run
...
def _state_payload(config: RuntimeConfig | None = None) -> dict[str, Any]:
    ...
    payload = cfg.to_dict(mask_secrets=True)
    payload["has_omdb_api_keys"] = bool((cfg.omdb_api_keys or "").strip())
    payload["run"] = get_run_status().get("run")
    return payload
...
def discover_plex(
    ...
    servers = discover_plex_servers(user_token)
    return {
        "servers": servers,
...
def plex_auth_status(session_id: str) -> dict[str, Any]:
    try:
        return poll_plex_auth_session(session_id)
    except KeyError:
