...
def discover_dlna_devices(
    ...
    timeout_s: float = (
        float(timeout) if timeout is not None else float(DLNA_DISCOVERY_TIMEOUT_SECONDS)
    )
    st_s: str = str(st) if st is not None else str(DLNA_DISCOVERY_ST)
    mx_i: int = int(mx) if mx is not None else int(DLNA_DISCOVERY_MX)

    device_desc_timeout_s: float = float(DLNA_DEVICE_DESC_TIMEOUT_SECONDS)
    device_desc_max_bytes: int = int(DLNA_DEVICE_DESC_MAX_BYTES)
    deny_tokens: list[str] = list(DLNA_DISCOVERY_DENY_TOKENS)
    allow_hint_tokens: list[str] = list(DLNA_DISCOVERY_ALLOW_HINT_TOKENS)
    max_responses: int = int(DLNA_DISCOVERY_MAX_RESPONSES)
    max_responses_per_host: int = int(DLNA_DISCOVERY_MAX_RESPONSES_PER_HOST)
