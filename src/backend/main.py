...
def start() -> None:
    ...
    try:
        ...
        if args.dashboard:
            update_run_progress(
                stage="dashboard",
                message="Opening the visual interface without running ingest.",
                message_key="run.message.dashboard_opening",
                record_event=True,
            )
            _run_web_dashboard()
            finish_run_progress(
                "succeeded",
                message="Visual interface opened.",
                message_key="run.message.dashboard_ready",
            )
            return
