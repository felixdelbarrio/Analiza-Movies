...
from backend.config_dlna import DLNA_SCAN_WORKERS
...
def analyze_dlna_server(
    ...
    update_run_progress(
        stage="connecting",
        message="Preparing DLNA server and video catalogue.",
        message_key="run.message.dlna_prepare_server",
        unit="containers",
        record_event=True,
    )
    ...
    try:
        ...
        update_run_progress(
            stage="connecting",
            message=f"Selected DLNA server: {server_label}.",
            message_key="run.message.dlna_selected_server",
            message_params={"server": server_label},
            scope=device.friendly_name,
            record_event=True,
        )
        ...
        update_run_progress(
            stage="scanning",
            message=f"{len(selected_containers)} containers ready to scan.",
            message_key="run.message.dlna_containers_ready",
            message_params={"count": len(selected_containers)},
            scope=chosen_root.title,
            current=0,
            total=len(selected_containers),
            unit="containers",
            counters={
                "containers_total": len(selected_containers),
        ...
        def _emit_item_progress(
            *,
            ...
            if (
                ...
                and current > 0
                and current % _PROGRESS_EVERY_N_ITEMS != 0
                and current != total
            ):
                return
        ...
        with (
            ...
            if not SILENT_MODE:
                ...
                for idx, container in enumerate(selected_containers, start=1):
                    ...
                    update_run_progress(
                        stage="scanning",
                        message=f"Scanning container {container.title} ({idx}/{total_containers}).",
                        message_key="run.message.dlna_container_scanning",
                        message_params={
                ...
                if total_candidates == 0:
                    ...
                    update_run_progress(
                        stage="finished",
                        message="DLNA completed without finding video items.",
                        message_key="run.message.dlna_no_items",
                        current=0,
                        total=0,
                        unit="items",
                        record_event=True,
                ...
                update_run_progress(
                    stage="analyzing",
                    message=f"{total_candidates} DLNA items ready for analysis.",
                    message_key="run.message.dlna_items_ready",
                    message_params={"count": total_candidates},
                    current=0,
                    total=total_candidates,
                    unit="items",
                    counters={
                        "containers_total": total_containers,
                        "containers_completed": total_containers,
                        "processed": 0,
                        "rows_written": 0,
                        "suggestions_written": 0,
                        "filtered_rows": 0,
                ...
                for container_title, items in candidates_by_container.items():
                    ...
                    _emit_item_progress(
                        message=f"Analyzing container {container_title} (items={len(items)}).",
                        message_key="run.message.dlna_container_analyzing",
                        message_params={
                            "container": container_title,
                            "count": len(items),
                        },
                        scope=container_title,
                        current=analyzed_so_far,
                        total=total_candidates,
                        record_event=True,
                    )
                        ...
                        while inflight_interactive:
                            ...
                            for fut in done:
                                ...
                                while next_to_write in pending_by_index:
                                    ...
                                    _emit_item_progress(
                                        message=(
                                            f"{container_title}: {total_items_processed}/{total_candidates} items processed."
                                        ),
                                        message_key="run.message.dlna_items_processed",
                                        message_params={
                                            "container": container_title,
            ...
            # ============================================================
                ...
                with (
                    ...
                    for idx, c in enumerate(selected_containers, start=1):
                        ...
                        update_run_progress(
                            stage="scanning",
                            message=f"Scanning container {c.title} ({idx}/{total_containers}).",
                            message_key="run.message.dlna_container_scanning",
                            message_params={
                                "container": c.title,
                                "index": idx,
                    ...
                    for sf in as_completed(scan_futures):
                        ...
                        update_run_progress(
                            stage="scanning",
                            message=f"Container {container_title} scanned; accumulated candidates: {total_candidates}.",
                            message_key="run.message.dlna_container_scanned",
                            message_params={
                                "container": container_title,
                                "count": total_candidates,
                            },
                            scope=container_title,
                            current=scanned_containers,
                            total=total_containers,
                            unit="containers",
                            counters={
                                "containers_total": total_containers,
                                "containers_completed": scanned_containers,
                                "processed": total_items_processed,
                                "rows_written": total_rows_written,
                                "suggestions_written": total_suggestions_written,
                                "filtered_rows": len(filtered_rows),
                ...
                update_run_progress(
                    stage="analyzing",
                    message=f"Streaming DLNA analysis ready with {total_candidates} candidates.",
                    message_key="run.message.dlna_streaming_ready",
                    message_params={"count": total_candidates},
                    current=total_items_processed,
                    total=total_candidates,
                    unit="items",
        ...
        if filtered_rows:
            update_run_progress(
                ...
                counters={
                    "containers_total": len(selected_containers),
                    "containers_completed": len(selected_containers),
                    "processed": total_items_processed,
                    "rows_written": total_rows_written,
                    "suggestions_written": total_suggestions_written,
                    "filtered_rows": len(filtered_rows),
        ...
        update_run_progress(
            stage="finished",
            message="DLNA completed. Reports are ready to explore.",
            message_key="run.message.dlna_ready",
            current=total_items_processed,
            total=total_items_processed if total_items_processed > 0 else None,
            unit="items",
            errors=total_items_errors,
            counters={
                "containers_total": len(selected_containers),
        ...
        update_run_progress(
            status="failed",
            stage="failed",
            message="DLNA analysis failed.",
            message_key="run.message.failed",
            errors=total_items_errors if "total_items_errors" in locals() else 0,
            record_event=True,
        )
        raise
    finally:
