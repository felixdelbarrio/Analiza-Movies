...
LOGS_ROOT = PROJECT_DIR / "logs" / "server"
...
def _build_logger_file_path() -> Path | None:
    ...
    if raw_path:
        p = Path(raw_path)
        resolved = p if p.is_absolute() else (PROJECT_DIR / p)
        _LOGGER_FILE_PATH_CACHED = resolved.resolve()
        return _LOGGER_FILE_PATH_CACHED

    raw_dir = _env_str("LOGGER_FILE_DIR", str(LOGS_ROOT)) or str(LOGS_ROOT)
    log_dir = _resolve_dir(raw_dir, base=PROJECT_DIR)
    prefix = (
